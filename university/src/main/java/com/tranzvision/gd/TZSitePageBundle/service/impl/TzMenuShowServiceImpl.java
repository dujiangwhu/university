...
public class TzMenuShowServiceImpl extends FrameworkImpl {
	...
	public String tzGetHtmlContent(String strParams) {
		...
		try {
			...
			//新闻的站点ID 和 栏目ID 应该是 配置出来的
			//String strSiteId = sqlQuery.queryForObject(
			//		"select TZ_HARDCODE_VAL from PS_TZ_HARDCD_PNT WHERE TZ_HARDCODE_PNT='BASE_SITE'", "String");
			//String strMenuId = sqlQuery.queryForObject(
			//		"select TZ_HARDCODE_VAL from PS_TZ_HARDCD_PNT WHERE TZ_HARDCODE_PNT='BASE_NEW_MENU'", "String");
			...
			if ((null != strSiteId && !"".equals(strSiteId)) && (null != strMenuId && !"".equals(strMenuId))) {
				...
				if (isMobile) {
					...
					if (mapSiteiInfo != null) {
						...
						orgId = mapSiteiInfo.get("TZ_JG_ID") == null ? ""
								: String.valueOf(mapSiteiInfo.get("TZ_JG_ID"));
					}
					...
					String title = sqlQuery.queryForObject(columnNameSQL, new Object[] { strSiteId, strColuId },
							"String");

					// 需要引入的js和css
					String jsCss = tzGDObject.getHTMLTextForDollar(
							"HTML.TZMobileWebsiteIndexBundle.TZ_M_ZL_TZ_LIST_JS_CSS", ctxPath, strSiteId, strColuId);
					// 主体部分
					String content = tzGDObject
							.getHTMLTextForDollar("HTML.TZMobileWebsiteIndexBundle.TZ_M_INDEX_HDLIST_HTML", title, "");

					indexHtml = tzGDObject.getHTMLTextForDollar("HTML.TZMobileWebsiteIndexBundle.TZ_MOBILE_BASE_HTML",
							title, ctxPath, strSiteId, orgId, strMenuId, jsCss, "", content, "");
					...
					if (null != strResultContent) {
						...
						String orgid = "";
						if (mapSiteOrg != null) {
							orgid = mapSiteOrg.get("TZ_JG_ID") == null ? ""
									: String.valueOf(mapSiteOrg.get("TZ_JG_ID"));
						}

						sql = "select TZ_MENU_NAME from PS_TZ_SITEI_MENU_T where TZ_SITEI_ID=? and TZ_MENU_ID=? and TZ_MENU_STATE='Y'";
						String strMenuName = sqlQuery.queryForObject(sql, new Object[] { strSiteId, strMenuId },
								"String");
