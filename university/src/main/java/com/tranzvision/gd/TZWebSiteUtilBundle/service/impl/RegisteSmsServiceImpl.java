...
public class RegisteSmsServiceImpl extends FrameworkImpl{
		...
		try{
			...
			if(jacksonUtil.containsKey("phone") 
				...
				strOrgid = jacksonUtil.getString("orgid").trim();
		      	...
		      	int count = jdbcTemplate.queryForObject(sql, new Object[]{strPhone,strOrgid.toLowerCase(),strSiteId},"Integer");
		      	if(count > 0){
		      		errorMsg[0] = "2";
		...
		try{
			...
			if(jacksonUtil.containsKey("phone") 
				      	...
				      	if(!"".equals(strYzm)){
				      		if(dtYzmValidDate.after(nowDate)){
				      			...
				      			if(dtYzmAddDate != null){
				      				...
				      				jdbcTemplate.update("UPDATE PS_TZ_SHJI_YZM_TBL SET TZ_EFF_FLAG='N' WHERE TZ_JG_ID=? AND TZ_MOBILE_PHONE=? AND TZ_CNTLOG_ADDTIME=? AND TZ_SITEI_ID=?", args);
				      			}
				      		}
				      	}
				...
				if(sendYzmFlag){
					...
					//给当前填写的手机号码发送验证码
					String strSmsContent = strSmsPrefix + strYzm;
					String strUserName = "";
			...
			if(jacksonUtil.containsKey("phone") 
				...
				strOrgid = jacksonUtil.getString("orgid").trim();
		      	...
		      	if(strPhone != null && !"".equals(strPhone)
			      	...
			      	Map<String, Object> yzmMap = jdbcTemplate.queryForMap(sql, new Object[]{strOrgid,strPhone,strYzm,strSiteId});
	...
	public String createPageForFixPass(String strParams){
		...
		try{
			...
			int count = jdbcTemplate.queryForObject(yzmSQL, new Object[]{strOrgid,strPhone,strYzm,strSiteId},"Integer");
	
			if(count> 0){
				//有效；
				if("ENG".equals(strLang)){
					str_content = tzGdObject.getHTMLText("HTML." + strHtmlFloder + ".TZ_GD_UPDATE_PWD_MB_ENG_HTML",strBeginUrl, strPhone, strLang, strOrgid,strStrongMsg, strNotice,contextPath,imgPath,loginUrl,strSiteId );
				}else{
					str_content = tzGdObject.getHTMLText("HTML." + strHtmlFloder + ".TZ_GD_UPDATE_PWD_MB_HTML",strBeginUrl, strPhone, strLang, strOrgid,strStrongMsg, strNotice,contextPath,imgPath,loginUrl,strSiteId );
				}
				...
				//无效；
				strBeginUrl = strBeginUrl + "?classid=enrollCls&siteid=" + strSiteId + "&orgid=" + strOrgid + "&lang=" + strLang + "&sen=" + strSenPara;
			...
			try{
				...
				if(jacksonUtil.containsKey("phone") 
					...
					strOrgid = jacksonUtil.getString("orgid").trim();
			      	...
			      	String tmpAccZt = jdbcTemplate.queryForObject(tSql, new Object[]{strPhone,strOrgid,strSiteId},"String");
			      	...
			      	if(count <= 0){
			      		...
			      		errorMsg[1] = validateUtil.getMessageTextWithLanguageCd(strOrgid, strLang,"TZ_SITE_MESSAGE", "134",
			      				"账号未绑定手机，请使用邮箱验证", "The number is not bound to the cell phone, please use the mailbox to verify.");
			            return strResult;
			      	}
