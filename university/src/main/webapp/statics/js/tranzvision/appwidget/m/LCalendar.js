...
window.LCalendar = (function() {
    ...
    MobileCalendar.prototype = {
        init: function(params) {
            ...
            this.bindEvent(this.type,params.maxDate);
        },
        bindEvent: function(type,maxDate) {
            var _self = this;
            //呼出日期插件
            ...
            //初始化年月日插件默认值
            function dateCtrlInit(maxDate) {
            	//console.log(maxDate);
            	var date = new Date();
            	//console.log(maxDate.split('-')[0]);
            	
            	if (parseInt(maxDate.split('-')[0]) > date.getFullYear()) {
            		maxDate='';
            	}
