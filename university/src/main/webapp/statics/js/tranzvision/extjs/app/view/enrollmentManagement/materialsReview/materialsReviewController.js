...
//由于新增弹出窗口会是当前页面不能操作，弹出窗口对应的tab即是当前活动的Tab
    ...
    startNewReview:function(btn){
        ...
        var startBtn = btn;
        //评议总人次，设置的评委评审总人次校验
        var requireCount = form.getForm().findField('requiredCount').getValue();
        var reviewAllCount = form.getForm().findField('reviewAllCount').getValue();

        if(reviewAllCount==requireCount){
        	if(startBtn.flagType === 'positive')
            {
                var tzParams = '{"ComID":"TZ_REVIEW_CL_COM","PageID":"TZ_CLPS_SCHE_STD","OperateType":"BUTTON","comParams":{"type":"reStartNewOnclick","classID":"' + classID + '","batchID":"' + batchID + '"}}';
                Ext.tzLoad(tzParams, function (respData) {
                    ...
                    var closeReviewButton = startBtn.findParentByType("form").down("button[name=closeReview]");
        	...
        	Ext.Msg.confirm('提示','当前班级批次，评委评审考生总人次不等于材料评审考生总人次，是否确认开启评审？',
            		function(btn){
            			if(btn=="yes"){            				
            				if(startBtn.flagType === 'positive')
            		        {
    ...
    closeReview:function(btn){
        ...
        if(btn.flagType === 'positive'){
            //可点击状态，设置setType值为1,当前按钮已点击
            tzParams = '{"ComID":"TZ_REVIEW_CL_COM","PageID":"TZ_CLPS_SCHE_STD","OperateType":"BUTTON","comParams":{"type":"closeReviewOnclick","classID":"' + classID + '","batchID":"' + batchID + '"}}';
            Ext.tzLoad(tzParams, function (respData) {
            });
            btn.setType = 1;
            //获取重新开始本轮评审按钮
...
//            }
    ...
    printPFZB: function(btn){
        ...
        if(checkLen == 0){
            ...
            var tzParams = '{"ComID":"TZ_REVIEW_CL_COM","PageID":"TZ_BMGL_CLPYSJ_STD","OperateType":"DCPY","comParams":{"TZ_CLASS_ID":"' + classID + '","TZ_APPLY_PC_ID":"' + batchID + '","TZ_PWEI_OPRIDS":' + JudgeJson + '}}';
