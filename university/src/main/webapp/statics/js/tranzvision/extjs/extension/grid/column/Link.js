...
Ext.define('tranzvision.extension.grid.column.Link', {
    ...
    /**
     * @cfg {Object} scope
     * The scope (`this` reference) in which the `{@link #handler}`, `{@link #getClass}`, `{@link #getText}`, `{@link #cfg-isDisabled}` and `{@link #getTip}` fuctions are executed.
    ...
    defaultRenderer: function(v, cellValues, record, rowIdx, colIdx, store, view) {
        ...
        for (; i < len; i++) {
			...
			text = item.text || (item.getText ? item.getText.apply(item.scope || scope, arguments) : null);
