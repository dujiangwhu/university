Ext.define('KitchenSink.store.Navigation', {
    ...
    constructor: function(config) {
        ...
        me.callParent([Ext.apply({
            root: {
                text: TranzvisionMeikecityAdvanced.Boot.languagePackage['tz-frmwrk-lang-00001'],
                id: 'all',
                expanded: true,
                children: config.returnMenu == true ? me.getChartNavItems() : me.getNavItems()
            }
        }, config)]);
    },

    addIconClasses: function (items,noReturnMenu)
    {
        var deletedIndex = -1;
        for (var item, i = items.length; i-- > 0; ) {
            ...
            if (!('iconCls' in item))
            {
                if(item.largeIcon != undefined && item.largeIcon != ' ' && item.largeIcon != '')
                {
                  if(item.returnMenu == true && noReturnMenu == false)
                  {
                    item.iconCls = 'icon-return-upper-menu';
                  }
                  else
                  {
            ...
            if (!('glyph' in item)) {
                ...
                item.glyph = '32@FontAwesome';
            }

            if (item.children) {
                this.addIconClasses(item.children,noReturnMenu);
            }

            if(item.returnMenu == true && noReturnMenu == true)
            {
              deletedIndex = i;
            }
        }
    ...
    {
        return this.addIconClasses(TranzvisionMeikecityAdvanced.Boot.mainMenu,false);
    ...
    {
       return this.addIconClasses(Ext.clone(TranzvisionMeikecityAdvanced.Boot.mainMenu),true);
    }
});
